...
*One MCP server. 21 databases. Zero context switching.*

Query PostgreSQL, MongoDB, Neo4j, Elasticsearch, Redis, and 16 more databases through a single unified interface. Built with ReScript, runs on Deno.
...
Modern applications use multiple databases — SQL for transactions, Redis for caching, Elasticsearch for search, vectors for AI, multimodal for knowledge federation. Switching between CLIs, APIs, and query languages is exhausting.

*Polyglot DB MCP* gives Claude (and other MCP clients) direct access to all 21 databases through natural language:
...
=== Option B: Local clone (Recommended for development)
...
=== Option C: Container (Recommended for production)
...
nerdctl run -d --name polyglot-db \
  ...
  ghcr.io/hyperpolymath/poly-db-mcp:latest
...
=== Option D: Deno Deploy (Serverless / HTTP Mode)
...
|db_list works
|Returns list of 21 supported databases
...
|*iTop* |PMPL 1.0 (FOSS) |IT asset management, CMDB |`itop_*`
|*VeriSimDB* |PMPL-1.0 (FOSS) |Multimodal federation (6 modalities: Graph, Vector, Tensor, Semantic, Document, Temporal), drift detection, ZKP verification |`verisimdb_*`
|===
...
[source,bash]
----
VERISIMDB_URL=http://localhost:8080
VERISIMDB_API_KEY=your-api-key     # optional
----
...
----
db_list              List all 21 supported databases
...
[quote]
Create a hexad with graph relationships and vector embeddings

[quote]
Search across all 6 modalities for machine learning papers

[quote]
Verify a zero-knowledge proof for citation compliance without accessing the data
...
|2.0.0 |Vision |100% ReScript, RSR Gold compliance, FormBD integration
|===

=== FormBD Integration (v2.0.0)

https://github.com/hyperpolymath/formbd[FormBD] is a narrative-first, reversible, audit-grade database. When its API stabilizes, poly-db-mcp will add:
...
*Primary:* link:LICENSE.txt[PMPL-1.0-or-later] (Palimpsest MPL).

*Fallback for registries that require a recognized SPDX license:* use **MPL-2.0-or-later** in metadata and add a short note that the project follows Palimpsest principles (see https://github.com/hyperpolymath/palimpsest-license[palimpsest-license]).

See link:LICENSE-NOTES.adoc[LICENSE-NOTES.adoc] for registry guidance.
...
OPSM Core
  |
  v
poly-db-mcp (database adapter layer for OPSM)
