...
|2.0.0 |Vision |100% ReScript, RSR Gold compliance, FormDB integration
|===

=== FormDB Integration (v2.0.0)

https://github.com/hyperpolymath/formdb[FormDB] is a narrative-first, reversible, audit-grade database. When its API stabilizes, poly-db-mcp will add:

* `formdb_query` — Execute FQL queries with provenance
* `formdb_explain` — Get plan + reasons for queries
* `formdb_introspect` — Schema and constraint inspection
* `formdb_journal` — Access audit trail and reversibility primitives
